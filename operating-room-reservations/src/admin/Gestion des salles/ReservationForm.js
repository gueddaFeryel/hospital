...
const ReservationForm = ({ editMode = false }) => {
    ...
    const checkConflict = async () => {
        if (!formData.roomName || !formData.startTime || !formData.endTime) return;
        ...
        try {
            const response = await axios.get('http://localhost:8086/api/reservations/check-conflict', {
                params: {
    ...
    const handleSubmit = async (e) => {
        ...
        if (hasConflict && !initialLoad) {
            toast.error("Cette salle est déjà réservée pour cette plage horaire");
            setIsLoading(false);
            return;
        }
                ...
                <form onSubmit={handleSubmit} className="modern-form" noValidate>
                    ...
                    {formData.roomName && formData.startTime && formData.endTime && (
                        <div className={`conflict-indicator ${
                            ...
                            {isCheckingConflict ? (
                                ...
                                <><i className="fas fa-clock"></i> Réservation expirée - Salle disponible</>
                            ) : hasConflict ? (
                                <><i className="fas fa-exclamation-triangle"></i> Conflit détecté</>
                            ) : (
                    ...
                    <div className="form-actions">
                        <button
                            ...
                            disabled={isLoading || (hasConflict && !initialLoad) || isReservationExpired()}
                        >
                            {isLoading ? (
